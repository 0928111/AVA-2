...
function combine_data(original: any, replacement: any): graphType {
  ...
  // To ensure a deep copy, we also need to make a deep copy of any nested objects or arrays
  // If currentNodes, visitedNodes, connectedPath are arrays or complex objects, they need to be handled separately
...
const ComponentsGraph: React.FC<Props> = ({ json, messageId }: Props) => {
  ...
  React.useEffect(() => {
    if (json) {
      ...
      const nodes = forcedata.nodes.map((d: any) => ({
        ...d,
      ...
      function boundingForce(width: number, height: number) {
        return (alpha: number) => {
          for (let d of nodess) {
            d.x = Math.max(
              node_radius,
              Math.min(width - node_radius, d.x as number),
            );
            d.y = Math.max(
              node_radius,
              Math.min(height - node_radius, d.y as number),
            );
          }
        };
      }
      ...
      function isHighlighted(node: any) {
        if (forcedata.currentNodes) {
          return forcedata.currentNodes.some(
      ...
      function isHighlightedLink(link: any) {
        if (forcedata.currentNodes) {
          return forcedata.currentNodes.some(
            (l: { source: any; target: any }) =>
              l.source === link.source.id && l.target === link.target.id,
          );
        }
      }

      function isConnectedLink(link: any) {
        if (forcedata.connectedPath) {
          const connectedPaths = forcedata.connectedPath;

          for (let i = 0; i < connectedPaths.length - 1; i++) {
            if (
              (link.source.id === connectedPaths[i] &&
                link.target.id === connectedPaths[i + 1]) ||
              (link.source.id === connectedPaths[i + 1] &&
                link.target.id === connectedPaths[i])
            ) {
              return true;
            }
          }
      ...
      function startAllAnimations() {
        ...
        svgT
          ...
          .duration(2000)
          .attr("r", (d: any) =>
            isHighlighted(d) ? node_radius + 5 : node_radius,
          )
          .attr("fill", (d: any) => (isHighlighted(d) ? "red" : d.color))
          .end()
          .then(() => {
            if (forcedata.connectedComponents) {
              if (forcedata.connectedComponents.length === 0) {
                startAllAnimations();
              } else {
                startConnectedComp();
              }
      ...
      function startConnectedComp() {
        if (forcedata.connectedComponents) {
          forcedata.connectedComponents.forEach((component, componentIndex) => {
            animateConnectedComp(
              component,
              nodes,
      ...
      function animateConnectedComp(
        paths: any[],
        nodes: any[],
        targetColor: string | d3.ColorCommonInstance,
      ) {
          ...
          setTimeout(() => {
            ...
            if (node) {
              const nodeElement = svg
                ...
                .node();
              const currentColor = nodeElement
                ? d3.select(nodeElement).style("fill")
                : "#1f77b4";
              ...
              if (index < paths.length - 1) {
                ...
                svgT
                  ...
                  .filter(
                    (d: any) =>
                      d.target.id === id ||
                      (d.source.id === id && d.target.id === nextId),
                  )
                  .transition()
                  .duration(1000)
                  .attrTween("stroke", function () {
                    const currentLinkColor: string = d3
                      .select(this)
                      .attr("stroke");
                    const interpolateLink = d3.interpolateRgb(
                      currentLinkColor,
                      targetColor,
                    );

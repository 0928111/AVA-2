...
const Graph_Cycle: React.FC<Props> = ({ json, messageId }: Props) => {
  ...
  React.useEffect(() => {
    if (json) {
      ...
      const nodes = forcedata.nodes.map((d: any) => ({
        ...d,
        color:
          forcedata.connectedPath && forcedata.connectedPath.includes(d.id)
            ? "#2ca02c"
            : "#1f77b4",
      }));
      ...
      function isHighlightedLink(link: any) {
        if (forcedata.currentDFSpaths) {
          const linkData = convertToLinksArray(
            forcedata.currentDFSpaths as string[],
          );

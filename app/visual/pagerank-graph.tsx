...
const PageRankGraph: React.FC<Props> = ({ json, messageId, currentStep = 0 }) => {
    ...
    renderTimeoutRef.current = setTimeout(() => {
      try {
        ...
        svg
          ...
          .attr("style", "max-width: 100%; height: auto; border: 1px solid #BFDBFE; border-radius: 16px; background: #ffffff; box-shadow: 0 4px 16px rgba(147, 197, 253, 0.15);");
        ...
        links.forEach((link1: any, i: number) => {
          links.forEach((link2: any, j: number) => {
            if (i !== j && link1.source.id === link2.target.id && link1.target.id === link2.source.id) {
              const pairKey = [link1.source.id, link1.target.id].sort().join('-');
        ...
        const link = svg.append("g")
          ...
          .attr("stroke-width", (d: any) => Math.max(1, Math.sqrt(d.weight || 1) * 1.5))
          .attr("fill", "none")
        ...
        if (json.currentIteration !== undefined) {
          svg.append("text")
            ...
            .attr("fill", "#1e293b")
            .style("transition", "all 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94)")
            .text(`Iteration: ${json.currentIteration}/${json.maxIterations || "∞"}`);
        }
        ...
        simulation.on("tick", () => {
          link.attr("d", (d: any) => {
            ...
            // 节点半径
            const nodeRadius = Math.max(12, Math.min(25, 12 + (d.source.rank || 0.15) * 30));
            const targetNodeRadius = Math.max(12, Math.min(25, 12 + (d.target.rank || 0.15) * 30));
            
            // 检查是否为双向边
            const isBidirectional = bidirectionalPairs.has([d.source.id, d.target.id].sort().join('-'));
